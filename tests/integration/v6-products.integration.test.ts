import { describe, it, expect } from 'vitest';
import { VintraceClient } from '../../src/client/VintraceClient';

const MOCK_BASE_URL = 'https://stoplight.io/mocks/vintrace/vintrace-server:v6/143865655';

function makeClient() {
  return new VintraceClient({
    baseUrl: MOCK_BASE_URL,
    organization: '',
    token: 'test-token',
    options: { maxRetries: 0 },
  });
}

describe('v6.products (integration)', { timeout: 15000 }, () => {
  it('getAll() returns a response from the mock server', async () => {
    const client = makeClient();
    const [data, error] = await client.v6.products.getAll();
    if (error) {
      expect(error).toBeDefined();
    } else {
      expect(data).toBeDefined();
    }
  });

  it('get() returns a response from the mock server', async () => {
    const client = makeClient();
    const [data, error] = await client.v6.products.get('1');
    if (error) {
      expect(error).toBeDefined();
    } else {
      expect(data).toBeDefined();
    }
  });

  it('create() returns a response from the mock server', async () => {
    const client = makeClient();
    const [data, error] = await client.v6.products.create({ batchCode: 'BCF01' });
    if (error) {
      expect(error).toBeDefined();
    } else {
      expect(data).toBeDefined();
    }
  });

  it('update() returns a response from the mock server', async () => {
    const client = makeClient();
    const [data, error] = await client.v6.products.update('1', {
      updateFields: [{ propertyType: 'description', propertyValue: 'Updated' }],
    });
    if (error) {
      expect(error).toBeDefined();
    } else {
      expect(data).toBeDefined();
    }
  });
});
